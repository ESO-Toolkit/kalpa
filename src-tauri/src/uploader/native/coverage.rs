...
pub const PROVEN_LINE_TYPES: &[&str] = &[
    // Empty until the live round-trip confirms server acceptance. The encoders for
    // STRUCTURALLY_READY_LINE_TYPES exist and are tested; only the empirical
    // confirmation is missing. See that list + the module docs for the flip.
];
