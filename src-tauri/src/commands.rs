...
pub fn list_backups(
    ...
    for entry in entries.flatten() {
        ...
        // Only classify a `char-`-named dir as Character when it actually carries
        // the v2 marker file — an unmarked `char-*` dir (e.g. a legacy/manual
        // backup routed there by `resolve_char_backup_name`) restores via the
        // WholeFile path (see classify_backup_for_restore), so it must display
        // as Manual to match, not claim a Character format it doesn't have.
