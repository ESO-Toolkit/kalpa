...
| R5 | pr-open | `fix/audit-r5-folder-qualified-conflicts` | [#393](https://github.com/ESO-Toolkit/kalpa/pull/393) (draft) | - | D-R5-1 | REVISE; both findings verified and fixed; follow-up pending | Rust 823 passed/17 ignored; Slint 768 passed/15 ignored; frontend `npm run check` + 490 vitest; clippy `-D warnings` both crates; fmt --check both; native build | Implements D-R5-1. Stacked on R4. Conflict wire **values** are folder-qualified (field names unchanged). |
