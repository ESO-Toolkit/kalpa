...
const tabsListVariants = cva(
  "group/tabs-list relative inline-flex w-fit items-center justify-center rounded-xl p-1 text-muted-foreground group-data-horizontal/tabs:h-9 group-data-vertical/tabs:h-fit group-data-vertical/tabs:flex-col data-[variant=line]:rounded-none",
  {
    variants: {
      variant: {
        default:
          "border border-structure-04 bg-structure-03 shadow-[inset_0_1px_2px_var(--scrim-15)]",
...
function TabsIndicator({ className, ...props }: TabsPrimitive.Indicator.Props) {
  return (
    <TabsPrimitive.Indicator
      ...
      className={cn(
        "absolute rounded-lg bg-structure-08 border border-structure-04 shadow-[0_1px_3px_var(--scrim-20),inset_0_1px_0_var(--structure-04)]",
...
function TabsTrigger({ className, ...props }: TabsPrimitive.Tab.Props) {
  return (
    <TabsPrimitive.Tab
      ...
      className={cn(
        "relative z-10 inline-flex h-[calc(100%-1px)] flex-1 items-center justify-center gap-1.5 rounded-lg border border-transparent px-2 py-1 text-xs font-medium whitespace-nowrap text-muted-foreground transition-colors duration-150 group-data-vertical/tabs:w-full group-data-vertical/tabs:justify-start hover:text-foreground hover:bg-structure-03 focus-visible:border-accent-sky/40 focus-visible:ring-[3px] focus-visible:ring-accent-sky/20 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
        ...
        "data-active:bg-structure-08 data-active:text-foreground data-active:border-structure-04 data-active:shadow-[0_1px_3px_var(--scrim-20),inset_0_1px_0_var(--structure-04)]",
