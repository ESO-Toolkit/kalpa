        ...
        {showCloseButton && (
          <DialogPrimitive.Close
            ...
            render={
              <Button
                variant="ghost"
                className="absolute top-4 right-5 size-7 rounded-lg border border-structure-06 bg-structure-04 text-muted-foreground/60 shadow-[inset_0_1px_0_var(--structure-04)] hover:bg-structure-10 hover:border-structure-12 hover:text-foreground active:scale-95 transition-all duration-150"
                size="icon-sm"
              />
            }
          >
...
function DialogHeader({ className, children, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      className={cn(
        "relative -mx-5 -mt-5 flex flex-col gap-1.5 border-b border-structure-06 bg-gradient-to-b from-structure-04 to-transparent pl-5 pr-12 pt-5 pb-4",
        className
      )}
      {...props}
    >
...
function DialogFooter({
  ...
  return (
    <div
      ...
      className={cn(
        "-mx-5 -mb-5 flex flex-col-reverse gap-2 rounded-b-2xl border-t border-structure-06 bg-gradient-to-b from-structure-02 to-transparent p-4 sm:flex-row sm:justify-end",
        className
      )}
      {...props}
    >
