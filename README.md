...
Every platform needs [Rust](https://rustup.rs/) (stable) and [Node.js](https://nodejs.org/). `engines.node` in `package.json` is the supported range, `.nvmrc` pins a known-good version for `nvm use`, and `npm run check:env` tells you whether yours qualifies.
